...
def test_from_env_basic_auth_cloud():
    """Test that from_env rejects cloud URLs with appropriate error message."""
    with patch.dict(
        os.environ,
        {
        ...
        with pytest.raises(
            ValueError,
            match="Xray is not supported in Atlassian Cloud.*Cloud URL detected: https://test.atlassian.net",
        ):
            XrayConfig.from_env()
...
def test_from_env_oauth_enable():
    """Test that from_env rejects OAuth configuration for Xray."""
    with patch.dict(
        os.environ,
        {
        ...
        # OAuth should be rejected for Xray since it's Cloud-only and Xray doesn't support Cloud
        with pytest.raises(
            ValueError,
            match="OAuth configuration detected for Xray, but Xray is only supported on Server/Data Center deployments",
        ):
            XrayConfig.from_env()


def test_from_env_projects_filter():
    """Test that from_env correctly loads projects filter configuration for Server/DC."""
    with patch.dict(
        os.environ,
        {
            "XRAY_URL": "https://xray.example.com",  # Server URL
...
def test_from_env_custom_headers():
    """Test that from_env correctly loads custom headers configuration for Server/DC."""
    with patch.dict(
        os.environ,
        {
            "XRAY_URL": "https://xray.example.com",  # Server URL
...
def test_from_env_proxy_settings():
    """Test that from_env correctly loads proxy environment variables for Server/DC."""
    with patch.dict(
        os.environ,
        {
            "XRAY_URL": "https://xray.example.com",  # Server URL
