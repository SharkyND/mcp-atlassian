...
class JiraIssue(ApiModel, TimestampMixin):
    ...
    def from_api_response(cls, data: dict[str, Any], **kwargs: Any) -> "JiraIssue":
        ...
        affects_versions = []
        if affects_versions_data := fields.get("versions"):
            if isinstance(affects_versions_data, list):
                affects_versions = [
                    str(version.get("name", ""))
                    if isinstance(version, dict)
                    else str(version)
                    for version in affects_versions_data
                    if version
                ]
    ...
    def to_simplified_dict(self) -> dict[str, Any]:
        ...
        if self.fix_versions and should_include_field("fix_versions"):
            result["fixVersions"] = self.fix_versions

        if self.affects_versions and should_include_field("versions"):
            result["affects_versions"] = self.affects_versions
            result["versions"] = self.affects_versions
