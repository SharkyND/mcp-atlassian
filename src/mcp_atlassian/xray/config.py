...
class XrayConfig:
    ...
    def from_env(cls) -> "XrayConfig":
        ...
        Returns:
            XrayConfig with values from environment variables

        Raises:
            ValueError: If required environment variables are missing or invalid
        """
        url = os.getenv("XRAY_URL")
        oauth_enable = os.getenv("ATLASSIAN_OAUTH_ENABLE", "").lower() in (
            "true",
            "1",
            "yes",
        )
        ...
        if not url and not oauth_enable:
            error_msg = "Missing required XRAY_URL environment variable"
        ...
        if is_cloud:
            error_msg = (
                ...
                f"Cloud URL detected: {url}. "
                f"Xray is only available on Server/Data Center deployments. "
                f"Please use a Server/Data Center URL instead."
            )
            raise ValueError(error_msg)

        if oauth_config:
            # OAuth is typically for cloud scenarios, but Xray doesn't support Cloud
            error_msg = (
                "OAuth configuration detected for Xray, but Xray is only supported on Server/Data Center deployments. "
                "Please use XRAY_PERSONAL_TOKEN or XRAY_USERNAME/XRAY_API_TOKEN for Server/Data Center authentication."
            )
            raise ValueError(error_msg)
            ...
            if personal_token:
                ...
                error_msg = "Server/Data Center authentication requires XRAY_PERSONAL_TOKEN or XRAY_USERNAME and XRAY_API_TOKEN"
                raise ValueError(error_msg)

        # SSL verification (for Server/DC)
        ssl_verify = is_env_ssl_verify("XRAY_SSL_VERIFY")

        # Get the projects filter if provided
        projects_filter = os.getenv("XRAY_PROJECTS_FILTER")

        # Proxy settings
        http_proxy = os.getenv("XRAY_HTTP_PROXY", os.getenv("HTTP_PROXY"))
        https_proxy = os.getenv("XRAY_HTTPS_PROXY", os.getenv("HTTPS_PROXY"))
        no_proxy = os.getenv("XRAY_NO_PROXY", os.getenv("NO_PROXY"))
        socks_proxy = os.getenv("XRAY_SOCKS_PROXY", os.getenv("SOCKS_PROXY"))

        # Custom headers - service-specific only
        custom_headers = get_custom_headers("XRAY_CUSTOM_HEADERS")
